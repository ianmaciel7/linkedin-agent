...
- [x] 1.1 Define the LinkedIn OAuth service contract, including structured inputs, safe outputs, and tool docstrings.
- [x] 1.2 Wire the OAuth service into the ADK app without expanding the scope beyond sign-in and read-only verification.
...
- [x] 2.1 Implement ADK-managed credential request, reuse, and persistence for the LinkedIn OAuth flow.
...
- [x] 3.3 Add or update an ADK evaluation case so the agent routes login requests to the LinkedIn OAuth service and not to unrelated tools.

## 4. Documentation and validation

- [x] 4.1 Update `README.md` and `.env.example` with the LinkedIn OAuth setup, required environment variables, and secret-handling notes.
